...
class Unit:
    ...
    def load(cls, raw):
        ...
        return cls(int(raw["id"]), kind, pos(raw["pos"]), int(raw["health"]),
                   ...
                   raw.get("targetTeam") or "",
                   int(raw.get("attackPower") or {"smallRobot": 5, "middleRobot": 10,
                       "largeRobot": 20, "bossRobot": 40}.get(kind, 0)))
...
class World:
    def __init__(self, request):
        ...
        self.team = team["type"]
        self.gold = int(team.get("goldNum") or 0)
        self.ours = [Unit.load(r) for r in team.get("roles") or [] if (r.get("health") or 0) > 0]
        self.enemy = [Unit.load(r) for r in (request.get("teamEnemy") or {}).get("roles") or [] if (r.get("health") or 0) > 0]
        self.robots = [Unit.load(r) for r in (request.get("robot") or {}).get("roles") or [] if (r.get("health") or 0) > 0]
        ...
        self.prices = {r["name"]: int(r.get("price") or 0) for r in request.get("vendorShopList") or []}
        self.shop = {r["name"]: int(r.get("price") or 0) for r in request.get("weaponShopList") or []}
        self.day_no = (self.round - 1) // 130 + 1
        self.pioneer = next((r for r in self.actors if r.kind == "pioneer"), None)
        self.tasks = team.get("playerTasks") or []
        self.threats = [r for r in self.robots if r.target_team == self.team]
