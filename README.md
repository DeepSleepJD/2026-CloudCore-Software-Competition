...
下载 [比赛提交包 CoreGeek.tar.gz](artifacts/CoreGeek.tar.gz) 的文件本体上传，勿上传 GitHub 整个仓库源码压缩包。详细行为和限制见 [v5 交付说明](docs/v5-delivery.md)，验证见 [v5 冒烟记录](production/qa/smoke-v5-2026-09-18.md)。
...
将 8080 替换为平台分配的端口。Python 3.10+，仅标准库，监听 `0.0.0.0`。日志输出到标准输出；GET 健康检查显示 `v5-jd`、请求数、最近回合、动作数及异常类型。

提交包仅包含 `main3.py`、`run.sh`、`agent/`、说明与构建信息。仓库中保留的 `zk_agent/`、`config.example.json`、`README-v2.md` 是旧版对照，不是此次部署入口；本版不读取旧配置。

## 本版变化

v5 新增题目文件预读、任务诊断、送修闭环、经济工人分工、夜间安全工作窗口和按收益比较投资。修复工人到墙边后仍反复移动；新接任务只有确认守炮人员到位才释放开拓者。详见 v5 交付说明。
